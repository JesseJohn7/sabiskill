...
const HomeTab: React.FC<HomeTabProps> = ({ onNavigate }) => {
    ...
    const fetchUser = async () => {
      ...
      if (user) {
        ...
        if (fullName) {
          ...
          const prefix = user.email.split("@")[0].replace(/[._\-0-9]/g, " ").trim().split(" ")[0];
  ...
  // ── Navigation helpers ────────────────────────────────────────────────────
  const handleExploreClick = () => onNavigate?.("explore");
  ...
  // Opens detail + records that the user opened the course
  ...
  // Called from inside CourseDetail — actually launches the video player
  const handleStartFromDetail = async (courseId: string) => {
    ...
    onNavigate?.("player"); // adjust to however you launch the video player
  };
  ...
  return (
    <div className="min-h-screen bg-slate-50 px-3 sm:px-4 md:px-6 lg:px-8 py-4 sm:py-6 md:py-8">
      ...
      {activeCourseId && !completedCourseIds.includes(activeCourseId) && (() => {
        ...
        const prog = progressMap[activeCourseId];
        return active ? (
            ...
            <div className="flex items-center gap-3 bg-blue-50 border border-blue-200 rounded-xl px-4 py-3">
              <div className="flex-1 min-w-0">
                <p className="text-xs sm:text-sm text-blue-700 font-semibold truncate">
                  You're on <span className="font-black">{active.title}</span>
                  {prog && (
                    <span className="ml-2 text-blue-500 font-medium">
                      · Lesson {prog.current_lesson + 1} of {prog.total_lessons} ({prog.progress_pct}%)
                    </span>
                  )}
                </p>
              </div>
      ...
      {hasRecents && (
        <div className="max-w-7xl mx-auto mb-7 sm:mb-8">
          <h2 className="text-base sm:text-lg md:text-xl lg:text-2xl font-black text-slate-800 mb-4 sm:mb-5">
            Recently Started
          </h2>
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-3 sm:gap-4">
            {recentCourses.map((c) => {
              ...
              const lessonLabel = prog
                ? `Lesson ${prog.current_lesson + 1} / ${prog.total_lessons}`
                : `${c.lessons} lessons`;
                ...
                >
                  <div className="relative overflow-hidden">
                    ...
                    <div className="absolute bottom-2 left-2 bg-slate-900/80 backdrop-blur-sm px-2 py-1 rounded-lg flex items-center gap-1">
                      ...
                      <span className="text-[10px] font-semibold text-white">{lessonLabel}</span>
                    </div>
                  </div>
                ...
                <div className="relative overflow-hidden">
                  ...
                  <div className="absolute bottom-2 left-2 bg-slate-900/80 backdrop-blur-sm px-2 sm:px-2.5 py-1 sm:py-1.5 rounded-lg flex items-center gap-1">
                    ...
                    <span className="text-[10px] sm:text-xs font-semibold text-white">
                      {prog ? `Lesson ${prog.current_lesson + 1}/${prog.total_lessons}` : `${c.lessons} lessons`}
                    </span>
                  </div>
                </div>
