...
import { useExploreActivity } from "./useExploreActivity";
...
export const ALL_COURSES = [
  ...
  },
  {
    ...
    longDescription:
      "Discover the art and science behind great digital products. Learn how to conduct user research, create wireframes and prototypes, and apply proven design principles to build interfaces people love.",
  ...
  },
  {
    ...
    longDescription:
      "Go beyond the basics with a thorough exploration of modern JavaScript. This course covers closures, prototypes, async/await, the event loop, and how to write clean, professional-grade code.",
  ...
  },
  {
    ...
    longDescription:
      "Get a solid, no-hype understanding of blockchain technology and the crypto ecosystem. Learn how Bitcoin and Ethereum work under the hood, explore DeFi protocols and NFTs.",
  ...
  },
  {
    ...
    longDescription:
      "Whether you're presenting to your team or speaking on a stage, this course gives you the tools to communicate with power and presence.",
  ...
  },
  {
    ...
    longDescription:
      "Transform your life by understanding the science of habits, motivation, and peak performance. This course blends psychology, neuroscience, and practical frameworks.",
...
function VideoPlayer({ videoId, title, autoPlay = false }: VideoPlayerProps) {
  ...
  return (
    <div className="relative w-full aspect-video bg-black rounded-t-2xl overflow-hidden">
      <iframe src={`https://www.youtube.com/embed/${videoId}?autoplay=1&rel=0&modestbranding=1`} title={title} allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture" allowFullScreen className="w-full h-full" />
    </div>
  );
}

// ── Review Form ───────────────────────────────────────────────────────────────
interface ReviewFormProps { courseId: string; onSubmit: (courseId: string, review: { name: string; rating: number; comment: string }) => void; }
function ReviewForm({ courseId, onSubmit }: ReviewFormProps) {
  ...
  return (
    <div className="space-y-4">
      ...
      <input type="text" placeholder="Your name" value={name} onChange={(e) => setName(e.target.value)} className="w-full border border-slate-200 rounded-xl px-4 py-2.5 text-sm text-slate-800 placeholder-slate-400 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent" />
      <textarea placeholder="Share your experience with this course..." value={comment} onChange={(e) => setComment(e.target.value)} rows={3} className="w-full border border-slate-200 rounded-xl px-4 py-2.5 text-sm text-slate-800 placeholder-slate-400 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent resize-none" />
      <button onClick={handleSubmit} disabled={!name.trim() || !rating || !comment.trim()} className="flex items-center gap-2 bg-blue-600 hover:bg-blue-700 disabled:bg-slate-200 disabled:text-slate-400 text-white font-semibold px-5 py-2.5 rounded-xl text-sm transition-all active:scale-95">
...
export function CourseDetail({ course, status, reviews, onBack, onStart, onReviewSubmit, backLabel = "Back to Explore", autoPlayVideo = false }: CourseDetailProps) {
            ...
            <div className="border-t border-slate-100 divide-y divide-slate-100">
              {course.topics.map((t, i) => (
                <div key={i} className="flex items-center gap-4 px-6 py-3">
                  ...
                  <span className="text-xs text-slate-400 font-medium">{Math.floor(Math.random() * 20 + 5)}:{String(Math.floor(Math.random() * 59)).padStart(2, "0")}</span>
                </div>
              ))}
            </div>
          )}
...
const ExploreTab: React.FC<ExploreTabProps> = ({ onNavigate }) => {
  ...
  // ── DB hooks ──────────────────────────────────────────────────────────────
  const { activeCourseId, completedCourseIds, openCourse, progressMap } = useCourseProgress();
  ...
  // Card click → record view in DB, open detail (no autoplay)
  const handleCardClick = async (id: string) => {
    await recordView(id); // 📝 saves to explore_views table
    setAutoPlayOnOpen(false);
    setSelectedCourseId(id);
  };

  // "Get Started" button → record view + open course in DB, open detail (autoplay)
  const handleStartFromGrid = async (courseId: string) => {
    const status = getStatus(courseId);
    if (status === "locked") return;
    const course = ALL_COURSES.find((c) => c.id === courseId);
    if (course) await openCourse(courseId, course.lessons); // 📝 saves to course_progress
    await recordView(courseId);
    setAutoPlayOnOpen(true);
    setSelectedCourseId(courseId);
  };

  // "Start Course" inside detail → also saves progress
  const handleStartFromDetail = async (courseId: string) => {
    const status = getStatus(courseId);
    if (status === "locked") return;
    const course = ALL_COURSES.find((c) => c.id === courseId);
    if (course) await openCourse(courseId, course.lessons);
          ...
          <div className="max-w-7xl mx-auto mb-5 sm:mb-6">
            <div className="flex items-center gap-3 bg-blue-50 border border-blue-200 rounded-xl px-4 py-3">
              <p className="text-xs sm:text-sm text-blue-700 font-semibold">
                You're currently on <span className="font-black">{active.title}</span>.
                {progressMap[activeCourseId] && (
                  <span className="ml-1 text-blue-500">· {progressMap[activeCourseId].progress_pct}% done</span>
                )}
              </p>
              <button
      ...
      <div className="max-w-7xl mx-auto">
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 2xl:grid-cols-5 gap-3 sm:gap-4 md:gap-5">
          {ALL_COURSES.map((c) => {
            ...
            const progress = prog?.progress_pct ?? (isCompleted ? 100 : 0);
              ...
              >
                <div className="relative overflow-hidden">
                  ...
                  <div className="absolute bottom-2 left-2 bg-slate-900/80 backdrop-blur-sm px-2 sm:px-2.5 py-1 sm:py-1.5 rounded-lg flex items-center gap-1">
                    ...
                    <span className="text-[10px] sm:text-xs font-semibold text-white">
                      {prog ? `${prog.current_lesson}/${prog.total_lessons} lessons` : `${c.lessons} lessons`}
                    </span>
                  </div>
                </div>
                ...
                <div className="p-4 sm:p-5 flex flex-col flex-1 space-y-3">
                  ...
                  <div className="mt-auto pt-1">
                    {isLocked ? (
                      <button onClick={(e) => { e.stopPropagation(); handleCardClick(c.id); }} className="w-full bg-slate-100 text-slate-500 font-semibold py-2.5 rounded-xl text-xs flex items-center justify-center gap-2 hover:bg-slate-200 transition-colors">
                        <Play className="w-3.5 h-3.5" /> Preview Course
                      </button>
                    ) : isCompleted ? (
                      <button onClick={(e) => { e.stopPropagation(); handleStartFromGrid(c.id); }} className="w-full bg-emerald-50 hover:bg-emerald-100 text-emerald-700 border border-emerald-200 font-semibold py-2.5 rounded-xl transition-all text-xs flex items-center justify-center gap-2">
                        <CheckCircle2 className="w-3.5 h-3.5" /> Review Course
                      </button>
                    ) : isActive ? (
                      <button onClick={(e) => { e.stopPropagation(); handleStartFromGrid(c.id); }} className="w-full bg-blue-600 hover:bg-blue-700 text-white font-semibold py-2.5 rounded-xl transition-all flex items-center justify-center gap-2 shadow-sm hover:shadow-md active:scale-95 text-xs">
